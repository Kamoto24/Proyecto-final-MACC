...
class Interfaz:
    ...
    def crearlosbotones(self, numero, escribir=True, ancho=7, alto=2):
        return Button(self.emergente, text=numero, width=ancho, height=alto, font=("Comic Sans MS", 10),
                      command=lambda: self.click(numero, escribir))
    ...
    def __init__(self, emergente):
        ...
        self.emergente.title("Calculadora")

        # Agregar una caja de texto para que sea la pantalla de la calculadora
        self.caja = Text(emergente, state="disabled", width=20, height=9, background="gold", foreground="midnight blue",
